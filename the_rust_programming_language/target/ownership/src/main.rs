...
// s1 and s2 are pointing to the same memory address, Rust would try to doubly free up memory which could
// lead to corruption. To avoid this, when we reassign variables like this Rust automatically considers s1 invalid
...
// this way heap data does get copied;


fn s_out_of_scope_main() {
...
// If we try to use s after the takes_ownership method; we will get a compile-time error;
