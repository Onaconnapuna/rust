...
// following 3 functions are the the same function, one with i32 and one with chars
...
// Here is how we can make a more generic funciton
...
// this however wont compile, needs std::cmp::PartialOrd because using > we dont necessarily know the types
...
// wont compile, notice i32 type and float 
struct Point<T> {
    x: T,
    y: T,
}
...
// if we need to do this we could just have multilple generic type parameters
struct Point<T, U> {
    x: T,
    y: U,
}
...
// In Enum definitions
// revisiting option T, which is a generic
enum Option<T> {
    Some(T),
    None,
}
...
//In fact, this is what we used to open a file in Listing 9-3, where T was filled in with the type std::fs::
//File when the file was opened successfully and E was filled in with the type std::io::Error 
